...
void shell_process_line(CommandLine *cl) {
    ...
    if (fgets(line, sizeof(line), stdin) && line[0] != '\n') {
        ...
        /* Check for '>>' for append redirection. */
        char *output_char = strchr(line, '>');
        if (output_char && output_char[1] == '>') {
            cl->output_append = 1;
        }

        /* Parse '>' */
        char *parsed_cmd = strtok(line, ">");
        if (output_char) {
            cl->output_file = strdup(strtok(NULL, " >"));
            line_ptr = parsed_cmd;
        }

        /* Let's first try to tokenize from '|'.
         * If second_cmd is NULL, there isn't any |. */
        char *first_cmd = strtok(line_ptr, "|");
